...
namespace TexLib
{
  ...
  public class TextureFont
  {
    ...
    public void WriteStringXY(string text, int X, int Y, int MaxWidth, int MaxHeight)
    {
        GL.MatrixMode(MatrixMode.Projection);
        GL.PushMatrix();
        Matrix4 ortho_projection = Matrix4.CreateOrthographicOffCenter(0, MaxWidth, 0, MaxHeight, -0.1f, 1.0f);
        GL.LoadMatrix(ref ortho_projection);
        GL.MatrixMode(MatrixMode.Modelview);
        GL.PushMatrix();
        GL.LoadIdentity();
        GL.Translate(X, (MaxHeight - Y) - 15, 0);
        GL.Scale(10, 10, 10);
