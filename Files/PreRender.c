...
Asm_VanillaHook(PreRender_SetValuesSave);
...
Asm_VanillaHook(PreRender_Init);
void PreRender_Init(PreRender* this) {
    bzero(this, sizeof(PreRender));
    ListAlloc_Init(&this->alloc);
}

Asm_VanillaHook(PreRender_SetValues);
...
Asm_VanillaHook(PreRender_Destroy);
 ...
 */
Asm_VanillaHook(PreRender_CopyImage);
 ...
 * `this->fbuf`.
 */
Asm_VanillaHook(PreRender_CopyImageRegionImpl);
 ...
 */
Asm_VanillaHook(PreRender_CoverageRgba16ToI8);
 ...
 */
Asm_VanillaHook(PreRender_SaveZBuffer);
 ...
 */
Asm_VanillaHook(PreRender_SaveFramebuffer);
 ...
 */
Asm_VanillaHook(PreRender_FetchFbufCoverage);
 ...
 */
Asm_VanillaHook(PreRender_DrawCoverage);
 ...
 */
Asm_VanillaHook(PreRender_RestoreZBuffer);
 ...
 */
Asm_VanillaHook(PreRender_RestoreFramebuffer);
 ...
 * `this->fbuf`.
 */
Asm_VanillaHook(PreRender_CopyImageRegion);
 ...
 */
Asm_VanillaHook(PreRender_AntiAliasFilter);
 ...
 * @param this  PreRender instance
 */
Asm_VanillaHook(PreRender_DivotFilter);
 ...
 * `this->cvgSave`
 */
Asm_VanillaHook(PreRender_ApplyFilters);
...
static u16 (*gWorkBuf)[SCREEN_WIDTH * SCREEN_HEIGHT];
#define motionBlurAlpha unk_12428[0]
static u8 sFirstDraw = true;
static u8 sInitialized = false;
extern void PreRender_MotionBlurOpaque(PreRender* this, Gfx** gfxP);
extern void PreRender_MotionBlur(PreRender* this, Gfx** gfxp, s32 alpha);
void Gameplay_DrawMotionBlur(PlayState* play) {
    if (!sInitialized)
    {
        GameState* gplay = (void*)play;
        gWorkBuf = GameAlloc_Malloc(&gplay->alloc, sizeof(*gWorkBuf) + 64 - 1);
        gWorkBuf = (void*)ALIGN64((u32)gWorkBuf);
        sInitialized = true;
    }
    if (play->motionBlurAlpha == 0) { // disabled
        sFirstDraw = true;
    } else {
        Gfx* gfxRef;
        Gfx* gfx;
        ...
        if (sFirstDraw) {
            ...
            PreRender_MotionBlur(&play->pauseBgPreRender, &gfx, (u8)play->motionBlurAlpha);
        }
